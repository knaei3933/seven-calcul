...
export function buildJapaneseChecklistItems(snapshot: CalculationChecklistSnapshot): ChecklistItem[] {
  ...
  add("basic.size", basic, "パウチサイズ", "入力されたパウチの左右幅・長さ・ピッチです。", `左右幅 = ${number(pouchWidth)} mm／長さ = ${number(pouchLength)} mm`, "入力値を確認します。", `入力サイズ = ${number(pouchWidth)} × ${number(pouchLength)} mm`, `${number(pouchWidth)} × ${number(pouchLength)}`, "mm");
  add("basic.pitch", basic, "製造ピッチ", "長さにシール等の付加寸法を加えた製造ピッチです。", `長さ = ${number(pouchLength)} mm／付加 = ${number(pitch && pouchLength ? D(pitch).minus(D(pouchLength)) : 0)} mm`, "長さ + 付加寸法", `${number(pouchLength)} + ${number(pitch && pouchLength ? D(pitch).minus(D(pouchLength)) : 0)} = ${number(pitch ?? 0)}`, number(pitch ?? 0), "mm");
  ...
  add("film.unit-price", film, "適用フィルム単価", "原反幅と発注長の価格帯から選択した単価です。", `原反幅 = ${number(snapshot.widthMm)}mm／発注長 = ${number(snapshot.film.orderLengthM)}m`, "価格表から選択", `適用帯 = ${snapshot.film.skuCosts[0]?.appliedBand ?? "-"}`, number(snapshot.film.unitPrice), "円/m");
  ...
  add("film.shipping-trips", film, "配送回数", "配送単位長で必要回数を切り上げます。", `製造換算長 = ${number(snapshot.film.orderLengthM)}m`, "製造換算長 ÷ 配送単位長 の切り上げ", "配送単位は原反幅設定を使用", String(snapshot.film.shippingTrips), "回");
  ...
  if (snapshot.gravure) {
    ...
    add("gravure.pattern-count", gravure, "発注パターン数", "必要長を納品パターン長で切り上げた回数です。", `必要長 = ${number(snapshot.film.requiredLengthM)}m／納品パターン = ${number(gp?.deliverablePatternLengthM ?? snapshot.deliverablePatternLengthM)}m`, "必要長 ÷ 納品パターン長 の切り上げ", "パターン条件を使用", String(snapshot.orderPatternCount), "回");
    add("gravure.material-width", gravure, "原反幅", "グラビア製造に適用する原反幅です。", `入力条件に基づく幅 = ${number(g.materialWidthMm)}mm`, "最小500mmまたはサイズ基準幅", `適用幅 = ${number(g.materialWidthMm)}mm`, number(g.materialWidthMm), "mm");
    add("gravure.production-length", gravure, "製作長", "発注パターンに対応する製作長です。", `パターン数 = ${snapshot.orderPatternCount}回／製作パターン = ${number(gp?.productionPatternLengthM ?? snapshot.productionPatternLengthM)}m`, "パターン数 × 製作パターン長", `${snapshot.orderPatternCount} × ${number(gp?.productionPatternLengthM ?? snapshot.productionPatternLengthM)}`, number(snapshot.productionPatternLengthM), "m");
    add("gravure.deliverable-length", gravure, "納品可能長", "ロスを除いて納品できる長さです。", `パターン数 = ${snapshot.orderPatternCount}回／納品パターン = ${number(gp?.deliverablePatternLengthM ?? snapshot.deliverablePatternLengthM)}m`, "パターン数 × 納品パターン長", `${snapshot.orderPatternCount} × ${number(gp?.deliverablePatternLengthM ?? snapshot.deliverablePatternLengthM)}`, number(snapshot.deliverablePatternLengthM), "m");
    add("gravure.film-cost", gravure, "フィルム製造費", "原材料・印刷・ラミネートを含む製造者販売価格の基礎です。", `製作長 = ${number(snapshot.productionPatternLengthM)}m／幅 = ${number(g.materialWidthMm)}mm`, "原材料費 + 印刷費 + ラミネート費 + 製造マージン", "グラビア価格パラメータを使用", number(g.customsBaseCostYen), "円");
