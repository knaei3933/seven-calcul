...
本システムの第一義的な利用者はカネイ貿易とする。カネイ貿易が顧客向け見積を作成・送付し、
...
```
1. カネイ貿易がパウチ仕様・充填条件・数量を入力する
2. システムが原価・推奨販売単価・見積書ドラフトを自動生成する
3. カネイ貿易が条件を確認・調整し、顧客へ見積を送付する
...
- 承認済み見積に対してカネイ貿易経由の成功報酬20%を内部計算・記録する
...
- 発行者はカネイ貿易、第一想定の宛先は株式会社セブン化学とする。
...
- 報酬額・報酬率はカネイ貿易およびシステム管理者のみ閲覧できる内部情報とする。
...
承認操作はカネイ貿易または管理者が行う。承認後に仕様変更が必要になった場合は、
