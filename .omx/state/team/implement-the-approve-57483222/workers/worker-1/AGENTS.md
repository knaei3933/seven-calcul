...
| --- | --- | --- | --- |
| Frontier (leader) | `glm-5.3-flash` | high | Primary leader/orchestrator for planning, coordination, and frontier-class reasoning. |
| Spark (explorer/fast) | `glm-5.3-flash` | low | Fast triage, explore, lightweight synthesis, and low-latency routing. |
| Standard (subagent default) | `glm-5.3` | high | Default standard-capability model for installable specialists and secondary worker lanes unless a role is explicitly frontier or spark. |
| `explore` | `glm-5.3-flash` | low | Fast codebase search and file/symbol mapping (fast-lane, fast) |
| `analyst` | `glm-5.3-flash` | medium | Requirements clarity, acceptance criteria, hidden constraints (frontier-orchestrator, frontier) |
| `planner` | `glm-5.3` | medium | Task sequencing, execution plans, risk flags (frontier-orchestrator, frontier) |
| `architect` | `glm-5.3` | xhigh | System design, boundaries, interfaces, long-horizon tradeoffs (frontier-orchestrator, frontier) |
| `debugger` | `glm-5.3` | high | Root-cause analysis, regression isolation, failure diagnosis (deep-worker, standard) |
| `executor` | `glm-5.3-flash` | medium | Code implementation, refactoring, feature work (deep-worker, standard) |
| `team-executor` | `glm-5.3-flash` | medium | Supervised team execution for conservative delivery lanes (deep-worker, frontier) |
| `verifier` | `glm-5.3` | high | Completion evidence, claim validation, test adequacy (frontier-orchestrator, standard) |
| `code-reviewer` | `glm-5.3-flash` | high | Comprehensive review across all concerns (frontier-orchestrator, frontier) |
| `dependency-expert` | `glm-5.3` | high | External SDK/API/package evaluation (frontier-orchestrator, standard) |
| `test-engineer` | `glm-5.3-flash` | medium | Test strategy, coverage, flaky-test hardening (deep-worker, frontier) |
| `designer` | `glm-5.3` | high | UX/UI architecture, interaction design (deep-worker, standard) |
| `writer` | `glm-5.3` | high | Documentation, migration notes, user guidance (fast-lane, standard) |
| `git-master` | `glm-5.3` | high | Commit strategy, history hygiene, rebasing (deep-worker, standard) |
| `code-simplifier` | `glm-5.3-flash` | high | Simplifies recently modified code for clarity and consistency without changing behavior (deep-worker, frontier) |
| `researcher` | `glm-5.3` | high | External documentation and reference research (fast-lane, standard) |
| `critic` | `glm-5.3-flash` | high | Plan/design critical challenge and review (frontier-orchestrator, frontier) |
| `vision` | `glm-5.3-flash` | low | Image/screenshot/diagram analysis (fast-lane, frontier) |
<!-- OMX:MODELS:END -->
...
You are a team worker in team "implement-the-approve-57483222". Your identity and assigned tasks are in your inbox file.
...
5. Read your task from <team_state_root>/team/implement-the-approve-57483222/tasks/task-<id>.json (example: task-1.json)
6. Task id format:
...
12. Update your status: write {"state": "idle", "updated_at": "<current ISO timestamp>"} to <team_state_root>/team/implement-the-approve-57483222/workers/{your-name}/status.json
13. Wait for new instructions (the lead will send them via your terminal)
14. Check your mailbox for messages at <team_state_root>/team/implement-the-approve-57483222/mailbox/{your-name}.json
...
Example:
omx team api send-message --input "{"team_name":"implement-the-approve-57483222","from_worker":"<your-worker-name>","to_worker":"leader-fixed","body":"ACK: <your-worker-name> initialized"}" --json
...
1. Read: `omx team api mailbox-list --input "{"team_name":"implement-the-approve-57483222","worker":"<your-worker-name>"}" --json`
2. Mark delivered: `omx team api mailbox-mark-delivered --input "{"team_name":"implement-the-approve-57483222","worker":"<your-worker-name>","message_id":"<MESSAGE_ID>"}" --json`
